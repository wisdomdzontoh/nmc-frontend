...
export default function Header({
  ...
  return (
    <header className="w-full border-b border-[#0D3B24] text-white shadow-lg" style={{ background: "linear-gradient(135deg, #0D3B24 0%, #1B5E3B 50%, #2E7D52 100%)" }}>
        ...
        <div className="flex items-center gap-3 min-w-0 flex-1">
          {onToggleSidebar && (
            <button
              ...
              className="mr-1 inline-flex h-9 w-9 items-center justify-center rounded-md bg-white/10 hover:bg-white/20 text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-[#1B5E3B] focus-visible:ring-white transition-colors"
          ...
          <Popover open={appsOpen} onOpenChange={setAppsOpen}>
            <PopoverTrigger asChild>
              <Button
                variant="ghost"
                size="icon"
                className="text-green-100 hover:bg-white/15 hover:text-white transition-all rounded-lg h-9 w-9"
                title="App launcher"
              >
            ...
            >
              {/* Search header */}
              <div className="p-4 border-b" style={{ background: "linear-gradient(135deg, #0D3B24, #1B5E3B)" }}>
                <p className="text-xs font-semibold text-green-200 mb-2 uppercase tracking-wide">Applications</p>
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-10 h-10 bg-white border-gray-200 focus-visible:ring-green-500"
                  />
                </div>
              </div>
                            ...
                            >
                              ...
                              <div>
                                <div className="flex items-center gap-2 mb-1">
                                  ...
                                  {app.badge && (
                                    <Badge className="text-[10px] px-1.5 py-0 h-4 bg-green-600 hover:bg-green-600">{app.badge}</Badge>
                                  )}
                                </div>
                        ...
                        {filteredApps.map((app) => (
                          <Link
                            ...
                            className={`flex items-center gap-3 px-4 py-3 transition-colors group ${
                              pathname.startsWith(app.href) ? "bg-green-50" : "hover:bg-gray-50"
                            }`}
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                <span className={`text-sm font-medium ${pathname.startsWith(app.href) ? "text-green-900" : "text-gray-900"}`}>
                                  {app.label}
                                </span>
                                {app.badge && (
                                  <Badge className="text-[10px] px-1.5 py-0 h-4 bg-green-600 hover:bg-green-600">{app.badge}</Badge>
                            ...
                            <ChevronRight className={`h-4 w-4 flex-shrink-0 ${pathname.startsWith(app.href) ? "text-green-600" : "text-gray-400"}`} />
                          </Link>
                        ))}
                      </div>
                    </div>
            ...
            <DropdownMenuTrigger asChild>
              <button className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-white/10 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-white/70 ml-1">
                <Avatar className="h-8 w-8 ring-2 ring-white/30">
                  ...
                  <AvatarFallback className="text-white text-xs font-bold" style={{ background: "linear-gradient(135deg, #0D3B24, #2E7D52)" }}>
                    {getUserInitials()}
                  </AvatarFallback>
                </Avatar>
                <div className="hidden lg:block text-left">
                  ...
                  <p className="text-[10px] text-green-200 leading-none mt-0.5">
              ...
              <DropdownMenuLabel className="pb-2">
                <div className="flex items-center gap-3">
                  <Avatar className="h-10 w-10">
                    <AvatarImage src="" alt={djangoUser?.full_name} />
                    <AvatarFallback className="text-white font-bold" style={{ background: "linear-gradient(135deg, #0D3B24, #2E7D52)" }}>
                      {getUserInitials()}
                    </AvatarFallback>
                  </Avatar>
                  <div className="flex-1 min-w-0">
